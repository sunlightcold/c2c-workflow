...
| 人工确认 | 本期不因实名不一致进入人工确认                             | 直接按支付方案创建支付订单                          |
| 支付     | 即时通道直接提交；批次通道进入 READY                       | 预检再次查询平台并校验状态、金额、币种、付款方式、收款账号和收款人姓名；平台实名差异仅记录展示，不阻断付款 |
...
历史版本曾对“实名不一致”的订单发送审核消息；当前版本不再发送此类二次确认消息：
...
当前版本不提供“确认下单 / 取消订单”二次确认按钮；实名不一致结果只在订单详情和创建通知中展示。

订单首次同步入库时，仍待付款且满足自动付款方案条件的订单直接进入自动支付流程。实名不一致不再触发群组审核消息、不要求人工确认；商家订单详情保留平台实名、收款人姓名、`identityMatched` 和 KYC 结果。实际付款前实时查询平台订单，继续复核订单状态、付款期限、金额、币种、付款方式和收款账号等资金关键资料；支付方案和作废规则保持不变。
...
自动支付异常使用数据库唯一键 `(tenantId, merchantId, code, referenceId)` 登记，只发送一次。支付订单创建失败使用平台订单号作为通知关联号，失败订单从自动建单候选中排除，不再重复尝试创建或重复通知；已进入支付流程的订单按既有状态回查规则处理。
...
8. 实名不一致不再触发二次确认；订单详情和自动创建通知保留核验字段。
