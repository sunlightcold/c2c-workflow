...
欧易不支持平台聊天和申诉，创建或更新欧易商家时，上述开关强制保持关闭，避免后台显示“已启用但永不执行”。日报直接查询欧易 `/v4/c2c/order/getOrderList` 的 `completed` 买入订单并分页聚合，不读取本地同步订单表。
...
2. 要求币安、商家启用、支付订单 `SUCCESS`、平台确认 `SUCCESS`、商家订单 `PENDING_RELEASE`。
3. 数据库原子认领，阻止机器人、后台、定时任务并发提交。
4. 实时查询币安订单，只有上游 `PAID` 继续。
5. 获取申诉原因；自动申诉固定选择原因码 `1`。
6. 通过支付通道获取支付宝回单 PDF。
7. PDF 转最多 5 张 JPG，获取币安预签名 URL 并上传。
8. 提交申诉并保存申诉单号。
...
| C2C付款超时自动申诉 | 30 秒 | 处理启用窗口内的币安超时待放行订单 |
