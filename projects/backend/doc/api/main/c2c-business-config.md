...
Telegram 通知按事件开关投递，并使用 HTML 格式。可自动付款且实名一致的新商家订单只进入自动支付流程，不发送发现通知；实名不一致、不可自动付款或订单异常时才发送人工介入提醒。待付款的人工介入消息提供“确认下单”和“作废订单”按钮；按钮点击时重新执行机器人、群组、商家、成员和 `C2C_ORDER_PAYMENT` 能力鉴权，并始终使用当前群绑定的商家范围。人工确认可放行实名不一致，但仍复核订单状态、金额、收款资料和付款截止时间，并在支付状态记录中保存操作来源。单笔支付仅在 `COMPLETED`、`FAILED`、`CANCELLED` 或 `FUND_EXCEPTION` 等终态发送，`SUBMITTING`、`PROCESSING`、`UNKNOWN` 和平台确认等待态静默。批量支付不发送批次子单的单笔结果，也不发送提交中/处理中的批次状态，只在成功、部分成功、失败、取消或异常终态发送一条汇总，并附失败明细；成功的单笔支付通知提供“获取回单”按钮。
