...
- 批次提交成功后不会立即回查。支付通道 Adapter 定义快速回查策略；支付宝批量有密当前为首次延迟 10 秒、间隔 5 秒、最多 12 次。达到次数后只结束快速回查，不改变批次、明细或支付订单状态，也不写入人工核实异常。
- 系统固定每 15 秒执行“自动回查支付结果”恢复任务。快速回查期间按 `nextReconcileAt` 到期时间处理；快速回查结束或旧数据的 `nextReconcileAt` 为空时，恢复任务仍持续查询所有未终态批次，直至支付宝返回明确成功或失败。
