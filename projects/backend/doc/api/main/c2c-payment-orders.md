...
列表默认第 1 页、每页 20 条，`pageSize` 最大 100；按创建时间倒序。`orderNo` 是订单号聚合搜索字段，可使用支付宝支付订单号 `upstreamId`、系统订单号 `paymentNo`、平台订单号 `sourceBusinessNo` 或支付批次号 `batchNo` 精确查询。聚合查询仍同时受所属单位、商家、来源、执行方式和状态条件限制。每条记录额外返回最近关联支付批次的 `batchNo`，未进入支付批次时为 `null`。`executionMode`、`sourceType` 和 `status` 必须使用系统枚举。组建支付批次时使用 `executionMode=BATCH` 查询批量有密候选订单。详情只在当前所属单位内查找，不存在或跨所属单位均返回 `404`。
...
- 商家订单已由当前支付订单锁定为支付处理中且允许付款，本地保存的金额、币种、收款方式、收款账号、转账姓名和平台付款方式可用。
- 系统实时读取交易平台订单详情，平台订单仍为待付款且允许付款，法币金额、币种、付款方式、收款账号和转账姓名与支付订单一致。资产数量、付款方式内部 ID、展示用收款姓名及实时 KYC 文本变化不单独阻止资金提交。
...
支付宝明确成功后，支付订单立即并持续保持 `SUCCESS`，商家订单进入 `PAID_PENDING_PLATFORM_CONFIRM`，独立的平台确认状态进入 `PENDING`。首次标记付款及重试均重新读取同一币安或欧易订单，核对订单编号与金额；若平台已经确认付款则直接收口，仍待付款时使用实时查询得到的付款方式 ID 调用平台“已付款”接口：币安传数字 `payId`，欧易传原字符串 `receiptAccountId`，不得传支付宝支付账号 ID。欧易执行前必须通过反欺诈检查。欧易标记后再次查询订单，只有状态为 `PAID` 或 `COMPLETED` 才把平台确认状态收口为 `SUCCESS`；币安标记接口明确成功后收口。商家订单分别进入 `PENDING_RELEASE` 或 `COMPLETED`。

平台确认接口失败或平台暂未显示已付款时，支付订单仍保持资金结果 `SUCCESS`，平台确认状态记录为 `FAILED`，后续只允许人工重试平台确认，不再调用支付宝。若超时 Worker 遗留 `PROCESSING`，15 秒任务只查询平台状态。若平台订单已经取消、过期或进入争议，支付订单才进入 `FUND_EXCEPTION`，商家订单进入资金异常。
