...
- `AG-002` 字段包括：代理商编码、名称、主体信息、联系人、时区、默认币种、状态、备注；代理商编码由系统按 `AGT` 前缀自动生成，不提供手工输入。
...
- `MER-002` 字段包括：商家编码、名称、所属单位、平台、外部商户 ID、主体资料、结算币种、时区、状态、联系人、风险等级、备注；商家编码由系统按 `MCH` 前缀自动生成，不提供手工输入。
...
- `MER-007` 商家通过支付方案使用本所属单位的多个支付账号；每条方案固定一个账号及该账号下的一个通道，并配置场景、币种、顺序、比例、自动付款和状态。执行方式由通道决定：单笔通道不得绑定批次策略，批量通道必须绑定当前商家可用的批次策略，包括所属单位全局策略或当前商家策略。
...
- `MER-012` 订单获取与付款配置相互独立。自动任务只使用开启自动付款的支付方案；操作员点击支付可使用任一启用方案。没有可用方案时保持商家订单待付款并明确提示，不创建支付订单。
...
| `PAYMENT_PROCESSING` | 支付订单已锁定并进入处理，资金请求可能尚未提交 | `PAID_PENDING_PLATFORM_CONFIRM`、`PENDING_PAYMENT`、`FUNDS_EXCEPTION` |
...
- `BO-010` 批次策略必须属于一个所属单位，范围分为“所属单位全局”和“指定商家”。全局策略对该所属单位下全部商家生效，商家策略只对指定商家生效，均不得跨所属单位使用。一个策略可包含多条启用规则；规则之间按 OR 并行判断，任一自动规则满足即可触发一次提交。
- `BO-011` 批次规则首期支持三种类型：手动提交、时间间隔、订单数量。手动提交不被自动任务触发；时间间隔从同一隔离范围内最早待付款订单开始计算；订单数量达到配置阈值即触发。
- `BO-012` 支付订单锁定批量支付方案时同步锁定批次策略。同一批次除所属单位、商家、支付账号、支付通道和币种外，批次策略也必须一致。
- `BO-013` 多条规则同时命中、任务重复扫描或多实例并发执行时，只允许认领并提交一个批次；规则命中信息和操作来源必须写入批次审计记录。
- `BO-014` 商家开启自动批次付款前，至少存在一条启用的批量支付方案，且该方案绑定已启用并包含有效规则的批次策略；只有手动规则时系统自动创建待付款订单但不自动提交批次。
- `BO-015` 全局策略只统一提交条件，不合并不同商家的订单。系统仍按所属单位、商家、批次策略、支付账号、支付通道和币种分别组批；手动提交全局策略时，也必须按商家分别生成支付批次。
...
- `PA-001` 新增、编辑、启用、停用支付账号；字段包括账号编码、名称、平台、商户号/账号标识、主体、网关、币种、时区和备注；账号编码由系统按 `PAC` 前缀自动生成，不提供手工输入。
...
匹配顺序为：所属单位与商家方案 → 自动/人工使用范围 → 业务场景 → 收款方式 → 币种 → 金额区间 → 账号及通道状态 → 主备顺序。同一顺序有多个候选时按配置比例分配；付款模式由最终选中的通道决定。
